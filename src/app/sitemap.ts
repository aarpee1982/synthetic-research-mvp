...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [...preserved.filter(entry => !paths.some(path => entry.url === `${baseUrl}${path}`)), ...paths.map(path => ({ url: `${baseUrl}${path}`, lastModified: new Date(`${providers.find(p => path === `/providers/${p.slug}`)?.reviewedAt ?? (path.startsWith("/synthetic-market-research-companies") || path === "/providers" || path.startsWith("/providers?") ? "2026-09-12" : path.startsWith("/news") || path === "" || path === "/providers/qualtrics" ? "2026-09-11" : "2026-09-06")}T00:00:00.000Z`) }))];
}
